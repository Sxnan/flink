...
public class TableImpl implements Table {
	...
	public Table leftOuterJoinLateral(Expression tableFunctionCall, Expression joinPredicate) {
		return joinLateralInternal(tableFunctionCall, Optional.of(joinPredicate), JoinType.LEFT_OUTER);
	}
	...
	public Table minus(Table right) {
		...
		return createTable(operationTreeBuilder.minus(operationTree, right.getQueryOperation(), false));
	}
	...
	public Table minusAll(Table right) {
		...
		return createTable(operationTreeBuilder.minus(operationTree, right.getQueryOperation(), true));
	}
	...
	public Table union(Table right) {
		...
		return createTable(operationTreeBuilder.union(operationTree, right.getQueryOperation(), false));
	}
	...
	public Table unionAll(Table right) {
		...
		return createTable(operationTreeBuilder.union(operationTree, right.getQueryOperation(), true));
	}
	...
	public Table intersect(Table right) {
		...
		return createTable(operationTreeBuilder.intersect(operationTree, right.getQueryOperation(), false));
	}
	...
	public Table intersectAll(Table right) {
		...
		return createTable(operationTreeBuilder.intersect(operationTree, right.getQueryOperation(), true));
	}

	@Override
	public Table orderBy(String fields) {
		return createTable(operationTreeBuilder.sort(ExpressionParser.parseExpressionList(fields), operationTree));
	}
	...
	public Table renameColumns(String fields) {
		return createTable(operationTreeBuilder.renameColumns(
	...
	public Table dropColumns(String fields) {
		return createTable(operationTreeBuilder.dropColumns(
			ExpressionParser.parseExpressionList(fields),
			operationTree));
	}
	...
	private static final class FlatAggregateTableImpl implements FlatAggregateTable {
		...
		private FlatAggregateTableImpl(
				TableImpl table,
				List<Expression> groupKey,
				Expression tableAggregateFunction) {
			this.table = table;
	...
	private static final class OverWindowedTableImpl implements OverWindowedTable {
		...
		public Table select(Expression... fields) {
			return table.createTable(
				table.operationTreeBuilder.project(Arrays.asList(fields), table.operationTree, overWindows)
			);
		}
	}
}
